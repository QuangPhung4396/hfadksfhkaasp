...
class InterstitialHandle: NSObject {
    ...
    // MARK: - properties
    private var _admobHandle = AdsInterstitial()
    var timeShowAds = 0
    // MARK: - initial
    ...
    func tryToPresent(_ block: @escaping () -> Void) {
        ...
        if DataObject.shared.extraFind("time_beetween_inter") == nil {
            timeBeetweenInter = 15
        } else {
            timeBeetweenInter = DataObject.shared.extraFind("time_beetween_inter")!!
        }
